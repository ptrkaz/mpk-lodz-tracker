...
- [ ] Within ~10 s, vehicle markers appear over Łódź. Trams are yellow, buses are magenta, line numbers render inside the marker.
...
- [ ] Tap a tram/bus marker → vehicles from the same line remain visible, the route polyline is highlighted in cyan, and markers with feed bearing show a direction arrow.
...
2. **Expand:** Drag handle up — sheet snaps to ~70 %. List of up to 20 stops appears, sorted by distance. Each row shows name, walk time, and distance.
...
8. **Favorite:** Tap the star in stop detail. Switch to Ulubione — the stop is listed there. Tap it — Map tab opens with that stop detail selected.
9. **Departures refresh:** Detail view auto-refreshes every 5 s. The "ostatnia aktualizacja" timestamp ticks. Closing detail returns to list and stops the 5 s polling.
10. **Delay:** A trip with >60 s delay shows `+N min` in red; an early trip shows `−N min` in green.
11. **Empty state:** If a stop has no upcoming departures (off-hours), detail shows "Brak nadchodzących odjazdów".
12. **Filter:** Toggle off a line in the filter sheet. Departures using that line disappear from the detail view; the nearby list itself is unchanged.
13. **Permission denied:** Revoke location permission in OS settings. Sheet auto-snaps to expanded and shows the CTA. Tap "Otwórz ustawienia" → opens system settings.
14. **No GPS fix:** Toggle airplane mode + location off and back; sheet shows "Czekam na sygnał GPS…" until a fix arrives. If a `lastFix` was previously persisted, it is used immediately instead of the waiting state.
15. **Background/foreground:** Background the app for >10 s during detail polling. Resume — detail refreshes immediately and timestamp updates. The map's 10 s tick also resumes. (Both pause and resume together via `AppLifecycleNotifier`.)
16. **Tab switch:** Switch to Lines, then back to Map. Sheet remounts; nearby list reappears; selected stop is NOT preserved.
